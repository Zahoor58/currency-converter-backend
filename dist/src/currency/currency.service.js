...
let CurrencyService = class CurrencyService {
    ...
    async historical(base, date) {
        ...
        if (requestedDate > oneDayAgo) {
            throw new common_1.HttpException('Historical data is only available for dates at least 1 day in the past. Please use the latest endpoint for recent rates.', common_1.HttpStatus.BAD_REQUEST);
        }
        const params = { date };
        if (base)
            params.base_currency = base;
